...
function BentoCard({ product, index, large }: { product: Product; index: number; large: boolean }) {
  ...
  return (
    <motion.div
      ...
      style={{ gridColumn: large ? 'span 7' : 'span 5', position: 'relative', borderRadius: '24px', overflow: 'hidden', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', cursor: 'pointer', aspectRatio: large ? '7/5' : '5/5' }}
    >
      {/* Image */}
      {product.image && (
        <motion.div
        ...
        >
          <Image src={product.image} alt={product.name} fill style={{ objectFit: 'cover' }} sizes="(max-width: 768px) 100vw, 50vw" />
        </motion.div>
      )}

      {/* Halftone overlay */}
      <div style={{ position: 'absolute', inset: 0, backgroundImage: 'radial-gradient(circle, #000 1px, transparent 1px)', backgroundSize: '4px 4px', opacity: 0.18, mixBlendMode: 'multiply', pointerEvents: 'none' }} />
      ...
      <motion.div
        ...
        style={{ position: 'absolute', inset: 0, background: 'rgba(10,10,10,0.55)', backdropFilter: 'blur(4px)' }}
      />

      {/* Badge */}
      {product.badge && (
        <div style={{ position: 'absolute', top: '1rem', left: '1rem', fontFamily: 'Inter, sans-serif', fontSize: '0.55rem', fontWeight: 500, letterSpacing: '0.2em', textTransform: 'uppercase', padding: '4px 10px', background: 'rgba(201,168,76,0.12)', color: '#C9A84C', border: '1px solid rgba(201,168,76,0.25)', borderRadius: '9999px', backdropFilter: 'blur(8px)' }}>
          {product.badge}
        </div>
      )}
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '1.5rem' }}>
        {/* Always visible */}
        <div style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.6rem', letterSpacing: '0.2em', color: 'rgba(201,168,76,0.6)', textTransform: 'uppercase', marginBottom: '0.35rem' }}>
          {product.desc}
        </div>
        <div style={{ fontFamily: '"Instrument Serif", serif', fontStyle: 'italic', fontSize: large ? '1.5rem' : '1.2rem', color: 'rgba(245,240,232,0.95)', marginBottom: '0.5rem' }}>
          {product.name}
        </div>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem' }}>
            <span style={{ fontFamily: '"Instrument Serif", serif', fontSize: '1.25rem', background: 'linear-gradient(135deg,#C9A84C,#E8C97A)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
              {product.price}
            </span>
            {product.originalPrice && (
              <span style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.72rem', color: 'rgba(245,240,232,0.3)', textDecoration: 'line-through' }}>{product.originalPrice}</span>
            )}
          </div>
          <motion.button
            onClick={handleAdd}
            ...
            whileTap={{ scale: 0.95 }}
            style={{ background: added ? 'rgba(76,201,120,0.12)' : 'rgba(245,240,232,0.95)', color: added ? '#4CC978' : '#0A0A0A', border: 'none', borderRadius: '9999px', padding: '0.5rem 1.2rem', fontFamily: 'Inter, sans-serif', fontSize: '0.7rem', fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap', transition: 'background 0.3s, color 0.3s' }}
          >
...
function ProductSection({ id, title, subtitle, emoji, category }: {
  id: string; title: string; subtitle: string; emoji: string; category: keyof typeof products;
}) {
        ...
        >
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.75rem' }}>
              ...
              <span style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.65rem', letterSpacing: '0.3em', textTransform: 'uppercase', color: 'rgba(201,168,76,0.6)', fontWeight: 400 }}>
                {subtitle}
              </span>
            </div>
            <h2 style={{ fontFamily: '"Instrument Serif", serif', fontStyle: 'italic', fontSize: 'clamp(2.2rem,5vw,3.8rem)', color: 'rgba(245,240,232,0.95)', lineHeight: 1, margin: 0 }}>
              {emoji} <em style={{ fontStyle: 'italic' }}>{title}</em>
            </h2>
          </div>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(245,240,232,0.6)', borderRadius: '9999px', padding: '0.65rem 1.5rem', fontFamily: 'Inter, sans-serif', fontSize: '0.72rem', fontWeight: 400, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.4rem' }}
          >
            View all {title} →
          </motion.button>
        </motion.div>

        {/* Bento Grid — 12 cols: 7/5 then 5/7 */}
    ...
    <div style={{ overflow: 'hidden', borderTop: '1px solid rgba(255,255,255,0.04)', borderBottom: '1px solid rgba(255,255,255,0.04)', padding: '0.75rem 0', position: 'relative', zIndex: 1 }}>
      <div style={{ display: 'flex', whiteSpace: 'nowrap', animation: 'marquee 30s linear infinite' }}>
        {Array(12).fill(null).map((_, i) => (
          <span key={i} style={{ fontFamily: '"Instrument Serif", serif', fontStyle: 'italic', fontSize: '0.9rem', fontWeight: 400, letterSpacing: '0.15em', color: 'rgba(201,168,76,0.18)', marginRight: '3rem' }}>
...
export default function ProductSections() {
  return (
    <>
      <div id="collections">
        <MarqueeDivider />
        <ProductSection id="watches"  title="Timepieces"      subtitle="Swiss Excellence"          emoji="⌚" category="watches" />
        <MarqueeDivider />
        <ProductSection id="perfumes" title="Fragrances"      subtitle="Rare Essences · Armaf"     emoji="🌸" category="perfumes" />
        <MarqueeDivider />
        <ProductSection id="suits"    title="Premium Shirts"  subtitle="Sartorial Mastery"         emoji="👔" category="shirts" />
        <MarqueeDivider />
      </div>
